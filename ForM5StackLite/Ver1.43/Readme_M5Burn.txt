━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Wifi_Rig_CW for M5ATOM Lite  Ver1.43
...
2通りの使い方ができます。
  ...
  ② USB 中継モード（Android アプリ Wifi_RIG_CTRL v1.43 との組み合わせ）
     ...
     Ver1.43 より SYNC トンネル機能に対応し、VPN 越し運用時の
     CW タイミング精度が向上しました。
     また、クライアントの Bluetooth 接続を BLE(Nordic UART Service)化し、
     Android 側でのペアリング操作なしで接続できるようになりました。


【ファームウェアの種類】
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  Clie_M5AtomLitev1.43.bin  クライアント用（電鍵を接続する側）
  Serv_M5AtomLitev1.43.bin  サーバー用（無線機のキー端子に接続する側）
...
必須項目:
  SSID        自宅 WiFi の SSID
  PASS        WiFi パスワード
  HOSTNAME    自身（このデバイス）のホスト名（mDNS 名前解決に使用）
...
【前提】
  Android スマートフォンに Wifi_RIG_CTRL for Android v1.43 をインストール済み
...
Wifi_Rig_CW is a CW keyer system using M5ATOM Lite that supports two operation modes:
  ...
  Mode 2: USB Relay (with Android app "Wifi_RIG_CTRL" v1.43)
    ...
    Ver1.43 adds SYNC tunnel support, improving CW timing accuracy over VPN.
    The client's Bluetooth link was also switched to BLE (Nordic UART Service),
    so Android can connect without a pairing step.


Firmware Files
--------------
  Clie_M5AtomLitev1.43.bin   Client firmware (connect your key to this unit)
  Serv_M5AtomLitev1.43.bin   Server firmware (connect to radio key terminal via optocoupler)
...
-----------------------
Requirements:
  Android smartphone with Wifi_RIG_CTRL v1.43 installed
  Raspberry Pi Zero 2W (set up with Wifi_RIG_CTRL)
...
- Source code is available in the ソースコード/Ver1.43/ folder.
- WiFi profile templates are in ソースコード/Ver1.43/Wifiプロファイル/.
...
- Raspberry Pi setup scripts and the Android APK are available at:
  https://github.com/ji1ore/M5CoreHamCAT/tree/main/v1.43/
